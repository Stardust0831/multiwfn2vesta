...
def _write_source_function_settings(
    ...
    *,
    ...
    normalized_mode = _normalize_source_function_mode(mode)
    ...
    if base_settings is not None and base_settings.exists():
        ...
        for index, line in enumerate(lines):
            if re.match(r"\s*srcfuncmode\s*=", line, flags=re.IGNORECASE):
                indent = re.match(r"\s*", line).group(0)
                lines[index] = f"{indent}srcfuncmode= {normalized_mode}"
                replaced = True
                break
        if not replaced:
            if lines and lines[-1].strip():
                lines.append("")
            lines.append(f"srcfuncmode= {normalized_mode}")
        path.write_text("\n".join(lines) + "\n", encoding="utf-8")
        return
    ...
    *,
    ...
    if function.index == 19:
        ...
        if reference_point is not None:
            raise ValueError("--reference-point is only valid for source-function")
        if _normalize_reference_unit(reference_unit) != "bohr":
            raise ValueError("--reference-unit is only valid for source-function")
        if source_function_mode is not None:
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    parser.add_argument(
        ...
        metavar=("X", "Y", "Z"),
        help=(
            "Reference point for function 19 source-function. Coordinates are "
            "Bohr by default, or Angstrom with --reference-unit angstrom."
        ),
    )
    parser.add_argument(
        "--reference-unit",
        ...
        if args.orbitals:
            ...
            if (
                ...
                raise ValueError(
                    ...
                    "--reference-point, --reference-unit, and "
