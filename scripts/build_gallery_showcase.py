...
READY_MAP_OUTPUT = DEFAULT_GALLERY / "feature_closure_ready_map.png"
...
READY_MAP_PANELS: Tuple[Tuple[str, str, str], ...] = (
    ("ag111_benzene_igmh_aim_front.png", "IGMH / IGM / mIGM", "Ag(111)+benzene periodic weak interaction"),
    ("ag111_benzene_igmh_aim_top.png", "AIM + IGMH", "One-cell periodic overlay and reusable camera"),
    ("gc_aim_overlay.png", "AIM topology", "GC hydrogen-bond paths and BCPs"),
    ("benzene_aim_overlay.png", "AIM PDB conversion", "Atoms-only paths.pdb / CPs.pdb display"),
    ("cdcl_nvt_trajectory_frame.png", "Trajectory frames", "extXYZ to VESTA frame style reuse"),
    ("cdcl_trajectory_frame.png", "Trajectory video", "Rendered PNG sequence to high-bitrate MP4"),
)
...
def build_ready_map(
    gallery_dir: Path = DEFAULT_GALLERY,
    output: Path = READY_MAP_OUTPUT,
    panels: Iterable[Tuple[str, str, str]] = READY_MAP_PANELS,
) -> Path:
    title_font = _font(34)
    ...
    header_h = 126
    footer_h = 72
    height = margin + header_h + rows * card_h + (rows - 1) * gap + footer_h + margin

    canvas = Image.new("RGB", (width, height), (242, 244, 247))
    draw = ImageDraw.Draw(canvas)
    draw.text(
        (margin, margin),
        "Ready multiwfn2vesta feature map",
        fill=(28, 36, 45),
        font=title_font,
    )
    draw.text(
        (margin, margin + 48),
        "Only features with real-system renders are shown as ready; implemented routes without figures remain needs-render.",
        fill=(75, 85, 99),
        font=subtitle_font,
    )
    ...
    for index, (filename, label, caption) in enumerate(panel_list):
        ...
        x = margin + col * (card_w + gap)
        y = margin + header_h + row * (card_h + gap)
        draw.rounded_rectangle((x, y, x + card_w, y + card_h), radius=8, fill=(255, 255, 255), outline=(207, 215, 224))
        path = gallery_dir / filename
        if not path.exists():
            draw.rectangle((x + 12, y + 12, x + card_w - 12, y + image_h), fill=(232, 236, 240))
            draw.text((x + 24, y + 110), f"missing: {filename}", fill=(120, 48, 48), font=label_font)
        else:
            image = _fit(Image.open(path), (card_w - 24, image_h - 12))
            canvas.paste(image, (x + 12, y + 12))
        text_y = y + image_h + 8
        draw.text((x + 18, text_y), label, fill=(31, 41, 55), font=label_font)
        draw.text((x + 18, text_y + 30), caption, fill=(91, 100, 112), font=small_font)

    footer_y = height - margin - footer_h + 10
    draw.text(
        (margin, footer_y),
        "Current gaps: grid-run scalar suites, atom coloring, Fukui/cube-arith, domain, STM, aIGM, and IRI clean render.",
        fill=(79, 88, 100),
        font=subtitle_font,
    )
    draw.text(
        (margin, footer_y + 32),
        "Use: multiwfn2vesta examples --closure-report",
        fill=(44, 62, 80),
        font=subtitle_font,
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--ready-map-output",
        type=Path,
        default=READY_MAP_OUTPUT,
        help="Output path for the status-oriented ready feature map.",
    )
    parser.add_argument(
        "--only",
        choices=["showcase", "ready-map", "both"],
        default="showcase",
        help="Which assembled figure to build.",
    )
    args = parser.parse_args()
    outputs = []
    if args.only in {"showcase", "both"}:
        outputs.append(build_showcase(args.gallery_dir, args.output))
    if args.only in {"ready-map", "both"}:
        outputs.append(build_ready_map(args.gallery_dir, args.ready_map_output))
